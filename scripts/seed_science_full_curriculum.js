...
let curriculum = {};
try {
    curriculum = require('./curriculum_science_data.js');
    console.log("✅ Loaded Science curriculum data.");
} catch (e) {
    console.log("❌ Failed to load Science curriculum data:", e.message);
    process.exit(1);
}
...
async function seedToFirestore() {
    console.log("\n🧪 Beginning full SCIENCE curriculum upload to Firebase...\n");
    let uploadCount = 0;
    const batchSize = 100;
    let batch = db.batch();
    ...
    for (const gradeNum of gradeNums) {
        ...
        const subjectId = `science-${gradeNum}`;
    ...
    console.log(`\n🎉 Upload complete! Pushed ${uploadCount} SCIENCE level modules to Firestore.\n`);
}
