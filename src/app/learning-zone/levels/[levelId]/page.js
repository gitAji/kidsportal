...
import LevelPath from '@/components/learning/LevelPath';

const LessonViewer = ({ task }) => {
  ...
  return (
    <div>
      ...
      >
        <FaArrowLeft className="mr-2" /> Back to Level
      </button>
    ...
    return (
        <div className="min-h-screen bg-gradient-to-b from-purple-100 to-teal-100 p-4 sm:p-8">
            <div className="max-w-7xl mx-auto">
                <button onClick={() => router.push('/learning-zone')} className="flex items-center text-lg font-semibold text-gray-700 hover:text-blue-600 mb-6">
                    <FaArrowLeft className="mr-2" /> Back to All Levels
                </button>
                ...
                <main className="bg-white/70 backdrop-blur-sm rounded-2xl shadow-lg p-8">
                    {activeTask && (
                        <>
                            {activeTask.type === 'lesson' && <LessonViewer task={activeTask} />}
