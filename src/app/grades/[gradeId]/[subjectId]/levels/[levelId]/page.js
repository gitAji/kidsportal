...
const TaskCard = ({ task }) => {
  ...
  return (
    <Link href={task.href} className="block p-6 rounded-lg shadow-lg text-center transform transition duration-300 hover:scale-105 bg-white hover:bg-gray-100 border-4 border-transparent hover:border-blue-400">
      <h3 className="text-2xl font-bold text-gray-800">{task.name}</h3>
      <div className={`mt-4 text-sm font-bold text-white px-3 py-1 rounded-full inline-block ${getStatusColor(task.status)}`}>
        {task.status}
      </div>
    </Link>
  );
...
const LevelPage = () => {
  ...
  useEffect(() => {
    const fetchedTasks = [
      { name: 'Introduction to Alphabets', href: `/grades/${gradeId}/${subjectId}/levels/${levelId}/tasks/alphabets`, status: 'Completed' },
      { name: 'Counting Numbers', href: `/grades/${gradeId}/${subjectId}/levels/${levelId}/tasks/counting`, status: 'In Progress' },
      { name: 'Simple Words', href: `/grades/${gradeId}/${subjectId}/levels/${levelId}/tasks/words`, status: 'Not Started' },
    ];
    ...
    <div className="min-h-screen bg-[var(--background)] text-[var(--foreground)] p-8">
      <div className="max-w-7xl mx-auto">
        ...
        <h1 className="text-5xl font-extrabold text-center mb-8 text-[var(--heading-color)] drop-shadow-lg">
          Level {levelId} - {subjectId.charAt(0).toUpperCase() + subjectId.slice(1)}
        </h1>
        <div className="bg-gray-800 rounded-lg p-6 mb-8 shadow-xl">
          <h2 className="text-2xl font-bold text-white mb-4">Your Progress</h2>
          <ProgressBar percentage={progress} />
          <p className="text-right text-white mt-2 text-lg">{progress.toFixed(0)}% Completed</p>
        </div>
