...
const questions = [
  {
    question: "What is the Tamil word for 'book'?",
    type: "mcq",
    options: ["பேனா", "புத்தகம்", "பென்சில்", "அழிப்பான்"],
    correctAnswer: "புத்தகம்",
    rewardPoints: 10,
  },
  {
    question: "Which of these is a Tamil number?",
    type: "mcq",
    options: ["One", "Two", "மூன்று", "Four"],
...
export default function Level1Content() {
  ...
  useEffect(() => {
    const savedProgress = localStorage.getItem("level1TamilProgress");
    if (savedProgress) {
  ...
  const handleReattempt = () => {
    ...
    setTimer(0);
    setTimerActive(false);
    localStorage.removeItem("level1TamilProgress");
  };

  const goToSubjectPage = () => {
    window.location.href = `/grades/2/tamil`;
  };
