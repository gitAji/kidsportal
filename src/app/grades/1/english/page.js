...
import ToggleModal from "../../../components/ui/Modal"; // Import ToggleModal component
import correctSound from "../../../../../public/sounds/correct.mp3"; // Correct sound
import incorrectSound from "../../../../../public/sounds/incorrect.mp3"; // Incorrect sound
...
const EnglishPage = () => {
  ...
  const navigateToLevel = (level) => {
    router.push(`/grades/1/english/level${level}`); // Navigate to the level page
    // Optional: Play sound when navigating
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          {progress.map((level, index) => (
            <div
              ...
              onClick={() => {
                navigateToLevel(index + 1); // Navigate to level on click
                if (level.status === "Not Attempted") {
                  playIncorrect(); // Play incorrect sound if not attempted
                } else {
                  playCorrect(); // Play correct sound if attempted
                }
              }}
            >
