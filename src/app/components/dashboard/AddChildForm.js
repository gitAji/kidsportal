...
import { collection, addDoc, doc, updateDoc, getDoc, query, getDocs } from 'firebase/firestore';
import { db, auth } from '../../../firebase/config';

const AddChildForm = ({ onClose, childToEdit, onSaveSuccess }) => {
  console.log("AddChildForm received childToEdit:", childToEdit);
  ...
  const [parentIsPremium, setParentIsPremium] = useState(false);
    ...
    const fetchParentData = async () => {
      if (auth.currentUser) {
        ...
        if (parentDocSnap.exists()) {
          setParentIsPremium(parentDocSnap.data().isPremium || false);
        }
        ...
        setCurrentChildCount(childrenSnapshot.size);
      }
    };
    fetchParentData();
  }, [childToEdit]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (childToEdit) {
        ...
        const maxChildren = parentIsPremium ? 3 : 5;
        if (currentChildCount >= maxChildren) {
          setError(`You have reached the maximum limit of ${maxChildren} children for your current plan.`);
          return;
        }
